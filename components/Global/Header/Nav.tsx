      ...
      {headerNavItems.map((item, index) => (
        <Link href={item.link} passHref={true} key={index}>
          <div className="flex items-center space-x-2 cursor-pointer group">
            ...
            <span className="relative uppercase text-sm font-medium before:bg-neutral-100 before:rounded-bl before:-bottom-1.5 before:h-0.5 before:inset-x-0 before:absolute before:transform before:origin-left before:scale-x-0 before:transition-all before:duration-200 group-hover:before:scale-x-100 hidden md:inline">
              {item.title}
            </span>
          </div>
        </Link>
